...
class AttNet(nn.Module):
    ...
    def build_network(self):
        ...
        self.bev_net_list2 = nn.ModuleList()
        self.bev_grid2point_list2 = nn.ModuleList()
        ...
        self.bev_net_list2.append(bird_view.BEVNet(bev_base_block, bev_context_layer, bev_layers))
        self.bev_grid2point_list2.append(get_module(bev_grid2point_list[0], in_dim=self.bev_net_list[0].out_channels[-1]))
        ...
        for n in range(1, self.stage_num):
            ...
            self.bev_net_list2.append(bird_view.BEVNet(bev_base_block, bev_context_layer, bev_layers))
            self.bev_grid2point_list2.append(get_module(bev_grid2point_list[n], in_dim=self.bev_net_list[n].out_channels[-1]))
    ...
    def stage_n_forward(self, point_feat, pcds_coord_wl, pcds_sphere_coord, stage_index=0):
        ...
        bev_input2 = VoxelMaxPool(pcds_feat=point_feat_tmp, pcds_ind=pcds_sphere_coord, output_size=(512,512), scale_rate=(1.0, 1.0))
        bev_feat_past2, bev_feat2 = self.bev_net_list2[stage_index](bev_input2)
        point_bev_feat2 = self.bev_grid2point_list2[stage_index](bev_feat2, pcds_sphere_coord)
        ...
        #merge multi-view
        if stage_index == 0:
            point_feat_out = self.point_post_list[stage_index](point_feat_tmp, point_bev_feat, point_bev_feat2)
            return point_feat_out
        else:
            point_feat_out = self.point_post_list[stage_index](point_feat, point_bev_feat, point_bev_feat2)
            return point_feat_out
    ...
    def forward_once(self, pcds_xyzi, pcds_coord, pcds_sphere_coord):
        ...
        pcds_sphere_coord = pcds_sphere_coord[:, :, :2].contiguous()
